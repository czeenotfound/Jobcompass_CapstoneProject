...
class Resumefilter(django_filters.FilterSet):
    ...
    employment_job_type = django_filters.ChoiceFilter(choices=Resume.employment_type_choices, label="Job Type",)
    location_job_type = django_filters.ChoiceFilter(choices=Resume.location_job_type_choices, label="Location Job Type",)
    ...
    class Meta:
        model = Resume
        fields = ['first_name','middle_name','last_name', 'job_position', 'address', 'employment_job_type', 'industry', 'expt_salary_min', 'expt_salary_max']  
    ...
    def filter_expt_salary_range(self, queryset, name, value):
        ...
        if value in expt_salary_ranges:
            ...
            return queryset.filter(expt_salary_min__gte=min_salary, expt_salary_max__lte=max_salary)
        return queryset